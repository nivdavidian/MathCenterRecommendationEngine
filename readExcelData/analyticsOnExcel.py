...
def calculate_cos_sim_by_country(c_code, l_code):
    ...
    df2 = pd.DataFrame(dbAPI.get_worksheet_grades_by_country_lang(c_code, l_code), columns = ["worksheet_uid"])
    
    uniq_pages = df2["worksheet_uid"].unique()
    
    zeroes_df = pd.DataFrame(0, index=uniq_pages, columns=uniq_topics)
...
# task("IL", "he")
# print(dbAPI.get_distinct_country_lang())
t = datetime.datetime.now()
