...
class TestRecall(unittest.TestCase):
    def __init__(self, methodName: str = "runTest") -> None:
        ...
        self.train_p = 0.85 # Train size in percentages
        self.N = 10 # recall recommendation size
    ...
    def test_markov_model(self):
        ...
        for (c_code, l_code) in cl_codes:
            ...
            train_size = int(self.train_p*len(users))
            ...
            import time
            t = time.time()
            predictions = [list(mixed_model.predict(x, markov_per=1, ps_per=0, mp_per=0, us_per=0).index) for x in test_X]
